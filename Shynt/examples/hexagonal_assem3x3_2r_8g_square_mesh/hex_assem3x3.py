...
import Shynt
from Shynt.api_py.Mesh.make_mesh import Mesh
...
inner_fuel = Material(
  "inner_fuel", composition=composition_inner_fuel, 
  options="sum tmp 453", color=(228, 237, 52)
)

helium_gas = Material(
  "helium_gas", 
  composition={"fractions":[(helium_03, 1.0E-05)],"type":"atom_density"}, 
  options="sum tmp 453", color=(252, 184, 235)
)

na_coolant = Material(
  "na_coolant", 
  composition={"fractions":[(na, 2.37718e-02)],"type":"atom_density"}, 
  options="sum tmp 453", color=(100, 141, 176))

composition_cladding = {
  "fractions": [
    (carbon_nat, 1.97913e-04), (si_28, 9.39458e-04), (si_29, 4.60789e-05), 
    (si_30, 2.93992e-05), (p_31, 4.60060e-05), (ti_46, 3.41106e-05), 
    (ti_47, 3.01070e-05), (ti_48, 2.92121e-04),    (ti_49, 2.09995e-05), 
    (ti_50, 1.97055e-05), (cr_50, 6.61142e-04), (cr_52, 1.22599e-02),
    (cr_53, 1.36391e-03), (cr_54, 3.33224e-04), (mn_55, 1.46981e-03), 
    (fe_54, 3.33120e-03),    (fe_56, 5.04275e-02), (fe_57, 1.14412e-03), 
    (fe_58, 1.49639e-04), (ni_58, 7.81396e-03),    (ni_60, 2.90969e-03), 
    (ni_61, 1.24406e-04), (ni_62, 3.90285e-04), (ni_64, 9.62720e-05), 
    (mo_92, 1.87735e-04), (mo_94, 1.15707e-04), (mo_95, 1.98193e-04), 
    (mo_96, 2.06406e-04),    (mo_97, 1.17638e-04), (mo_98, 2.95822e-04), 
    (mo_100, 1.16718e-04)
  ],
  "type": "atom_density"
}
...
cladding = Material(
  "cladding", composition=composition_cladding, 
  options="sum tmp 453", color=(108, 112, 115)
)


# Defining pins  -------------------------------------------------------
p_i = Pin("pin_inner")
mat_lev = [inner_fuel, na_coolant]
rad_lev = [0.35700, None]
...
assembly_hex = HexagonalLatticeTypeX(
  "hex_assem", (0.0, 0.0), 0.98259, lattice_hex
)

hex_wrap = InfiniteHexagonalCylinderYtype(
  0, 0, 1.4182465, boundary="reflective"
)

# Main problem cell
assembly_cell = Cell(
  "hex_assembly_problem", region=-hex_wrap, fill=assembly_hex
)
outside_cell = Cell("outside_world", region=+hex_wrap)

mesh_assembly = Mesh(
  assembly_cell, 
  global_mesh_type="square_mesh_pin_no_J_share", 
  local_mesh_type="cell"
)



# Montecarlo params and libraries --------------------------------------
mc_params = MontecarloParams(1500, 1000, 500) 
