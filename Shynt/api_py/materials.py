...
class Material:
  ...
  addIsotope(isotope, mass_fraction=None, atom_fraction=None, atom_density=None)
  ...
  def __str__(self):
    ...
    for iso in self.__isotopes:
      return_statement += "\t\t%s\n"%iso
    return f"<class Material(name=\'{self.__name}\') >"
...
class Isotope:
  ...
  """
  A class used to represent an isotope
  ...
  ----------
  name: str
    The name of the isotope as represented in Serpent

  Methods
  -------
  serpent_syntax()
    Gives a string with the serpent syntax
  """
