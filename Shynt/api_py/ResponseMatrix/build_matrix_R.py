...
def getResponseMatrix_allG(mesh_info, energy_g, probabilities):
  """
  Builds the Response Matrix (mR) for the system for each energy group
  ...
  for n_id in coarse_nodes:
    ...
    responseMatrix_byCoarse.append(rm_n)
  big_mR = getBlockMatrix(responseMatrix_byCoarse)
    
          
  return big_mR

def build_R_allG(probabilities, surfaces, surface_areas, energy_g):
  """
  Builds the response matrix R for a coarse node
  ...
  for g in range(energy_g):
    ...
    #     area_b = surface_areas[surf_b_id]
    #     p_b_a = probabilities["surfaces"][surf_b_id]["surfaces"][surf_a_id][g]
    #     mRg[a][b] =  area_b * p_b_a / area_a
    mRg = build_R(
      probabilities, surfaces, surface_areas, g
    )
    matrices_R.append(mRg)
  big_mR = getBlockMatrix(matrices_R)
  return big_mR
