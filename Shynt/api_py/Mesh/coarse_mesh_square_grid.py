...
class SquareGridMeshHexAssembly(CoarseMesh):
  ...
  In this class the following variables are obtained:
    - symmetry and similarity of the coarse nodes
    - coarse node_map: 
    - coarse nodes: dictionary with each coarse node as a cell 
                    with the information needed, volumes, surfaces areas,
                    regions and surfaces id_s.
  ...
  def fill_nodes_surfaces(self):
    ...
    for n_id, surfs in self.nodes_surfaces.items():
      for surf_dir, data in surfs.items():
        if data is not None: 
  ...
  def fix_boundaries(self, nodes_surfaces):
    ...
    def get_position_in_hexagon(point):
      ...
      given a point of the top side of the rectangle
      """
      x, y = point
  ...
  def calculate_surfaces_twins(self):
    """It analyzes the nodes and found the twins of every surface of each node
    by 
    ...
    for nid, eq_array in self.equivalent_nodes.items():
      ...
      for eq_nid in eq_array:
        ...
        for region in regions_ids[nid]:
          ...
          coarse_nodes_regions[eq_nid].append(f"{eq_nid}_{region[1]}")
