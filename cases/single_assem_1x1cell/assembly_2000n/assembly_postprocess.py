...
regions_diagonal_hybrid = postprocess.regions_to_plot(model_universe, line="diagonal", code="hybrid")
...
for r in regions_diagonal_serp:
    ...
    for g in range(energy_g):
        ...
        serp_errors_to_plot[g].append(serp_errors[g][s_idx])
...
plt.plot(shynt_array_to_plot[0], label="Hybrid")
# plt.ylim([0.02, 0.17])
plt.title("Thermal group")
plt.legend()
# plt.savefig("Thermal group")
plt.show()

plt.figure()
plt.errorbar(x, serp_array_to_plot[1], yerr=serp_errors_to_plot[1], label="Serpent")
plt.plot(shynt_array_to_plot[1], label="Hybrid")
# plt.ylim([0.2, 1.4])
plt.title("Fast group")
plt.legend()
# plt.savefig("Fast group")
plt.show()
